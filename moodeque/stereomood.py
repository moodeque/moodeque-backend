...
class StereoMoodClient(object):
  ...
  SEARCH_URL = "http://www.stereomood.com/api/search.json?q=%s&type=%s"
  ...
  def search_by_mood(self, mood):
    return self.search_by('mood', mood)

  def search_by_activity(self, activity):
    return self.search_by('activity', activity)

  def search_by_site(self, site):
    return self.search_by('site', site)

  def search_by(self, type, q):
    if type not in ('mood', 'activity', 'site'):
      return None

    response = self.client.get(self.SEARCH_URL % (quote_plus(q), type))
